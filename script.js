...
var generateScaleMap = function(scale, wh) {
  ...
  var imageData = new ImageData(data, wh, wh);
...
var parsedIGCToMesh = function(parsedIGC, material) {
  ...
  var alt = new Float32Array(JSONpath.length * 6);
...
var makeGraph = function(igc) {
  ...
  var margin = {
    ...
    width = boxWidth * 0.718 - margin.left - margin.right,
...
function init() {
  ...
  var light = new THREE.HemisphereLight(0xa6d4ff, 0x080600, 2);
  scene.add(light);

  var light = new THREE.DirectionalLight(0xffeab8, 4);
  ...
  sphereInter = new THREE.Line(geometry, material);
  sphereInter.name = "indicator";
  var light = new THREE.PointLight(0xfffeed, 1, 200, 2);
  light.name = "indicatorlight";
  sphereInter.add(light);
  sphereInter.material.depthTest = true;
  sphereInter.material.side = THREE.DoubleSide;
